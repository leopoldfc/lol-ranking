/**
 * Scrape LPL 2025 depuis gol.gg
 * Splits : Split 1 + Split 1 Playoffs + Split 2 + Split 2 Playoffs + Split 2 Placements + Split 3 + Regional Finals + Grand Finals
 * Combinés : Split 1 Combined, Split 2 Combined, Combined annuel
 * Export → frontend/public/leagues/lpl-2025/export.json
 *
 * Usage : npx tsx leagues/2025/lpl-2025/scrape.ts
 */
...
const SPLITS = [
  { key: 'split1',      name: 'LPL 2025 Split 1',           season: 'ALL' },
  { key: 'split1po',    name: 'LPL 2025 Split 1 Playoffs',  season: 'ALL' },
  { key: 'split2',      name: 'LPL 2025 Split 2',             season: 'ALL' },
  { key: 'split2po',    name: 'LPL 2025 Split 2 Playoffs',    season: 'ALL' },
  { key: 'split2place', name: 'LPL 2025 Split 2 Placements',  season: 'ALL' },
  { key: 'split3',      name: 'LPL 2025 Split 3',           season: 'ALL' },
  { key: 'regionals',   name: 'LPL 2025 Regional Finals',   season: 'ALL' },
  { key: 'grandfinals', name: 'LPL 2025 Grand Finals',      season: 'ALL' },
];
const COMBINED_NAME = 'LPL 2025';
...
const split1CombPlayers = players.filter(p => p.rows.split1 || p.rows.split1po);
split1CombPlayers.forEach(p => {
  p.split1Combined = makeCombined([p.rows.split1, p.rows.split1po].filter(Boolean));
});

// Split 2 Combined (Split 2 + Playoffs + Placements)
const split2CombPlayers = players.filter(p => p.rows.split2 || p.rows.split2po || p.rows.split2place);
split2CombPlayers.forEach(p => {
  p.split2Combined = makeCombined([p.rows.split2, p.rows.split2po, p.rows.split2place].filter(Boolean));
});

// Split 3 Combined (Split 3 + Regional Finals + Grand Finals)
const split3CombPlayers = players.filter(p => p.rows.split3 || p.rows.regionals || p.rows.grandfinals);
split3CombPlayers.forEach(p => {
  p.split3Combined = makeCombined([p.rows.split3, p.rows.regionals, p.rows.grandfinals].filter(Boolean));
});
...
const s1CombResults = computeRatingsForGroup(split1CombPlayers, p => p.split1Combined);
split1CombPlayers.forEach((p, i) => { p.lirSplit1Combined = s1CombResults[i]; });

const s2CombResults = computeRatingsForGroup(split2CombPlayers, p => p.split2Combined);
split2CombPlayers.forEach((p, i) => { p.lirSplit2Combined = s2CombResults[i]; });

const s3CombResults = computeRatingsForGroup(split3CombPlayers, p => p.split3Combined);
split3CombPlayers.forEach((p, i) => { p.lirSplit3Combined = s3CombResults[i]; });

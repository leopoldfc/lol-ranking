/**
 * Scrape LFL 2025 depuis gol.gg
 * Splits : Spring Split + Spring Playoffs + Summer Split + Summer Playoffs
 * Combinés : Spring Combined, Summer Combined, Combined annuel
 * Export → frontend/public/leagues/2025/lfl-2025/export.json
 *
 * Usage : npx tsx leagues/2025/lfl-2025/scrape.ts
 */
...
const SPLITS = [
  { key: 'spring',    name: 'LFL 2025 Spring Split',    season: 'ALL' },
  { key: 'springpo',  name: 'LFL 2025 Spring Playoffs', season: 'ALL' },
  { key: 'summer',    name: 'LFL 2025 Summer Split',    season: 'ALL' },
  { key: 'summerpo',  name: 'LFL 2025 Summer Playoffs', season: 'ALL' },
];
const COMBINED_NAME = 'LFL 2025';
...
const exportData = {
  metadata: {
    ...
    tournaments: [{ name: COMBINED_NAME, league: 'LFL', year: 2025, split: 'Full Year', scrapedAt: new Date().toISOString() }],
  },
  ...
  players: players.map(p => {
    ...
    if (p.springCombined) {
      ...
      tournaments['LFL 2025 Spring'] = { ...toStats(p.springCombined), team: springTeam, ...(p.lirSpringCombined ?? {}) };
    }
    if (p.summerCombined) {
      const summerTeam = p.rows.summerpo?.team || p.rows.summer?.team || '';
      tournaments['LFL 2025 Summer'] = { ...toStats(p.summerCombined), team: summerTeam, ...(p.lirSummerCombined ?? {}) };
    }
...
fs.writeFileSync(path.join(__dirname, '../../../frontend/public/leagues/2025/lfl-2025/export.json'), JSON.stringify(exportData, null, 2));

/**
 * Scrape LEC 2025 depuis gol.gg
 * Splits : Winter + Winter Playoffs + Spring + Spring Playoffs + Summer + Summer Playoffs
 * Combinés : Winter Combined, Spring Combined, Summer Combined, Combined annuel
 * Export → frontend/public/leagues/lec-2025/export.json
 *
 * Usage : npx tsx leagues/2025/lec-2025/scrape.ts
 */
...
const SPLITS = [
  { key: 'winter',    name: 'LEC Winter 2025',            season: 'ALL' },
  { key: 'winterpo',  name: 'LEC 2025 Winter Playoffs',   season: 'ALL' },
  { key: 'spring',    name: 'LEC 2025 Spring Season',     season: 'ALL' },
  { key: 'springpo',  name: 'LEC 2025 Spring Playoffs',   season: 'ALL' },
  { key: 'summer',    name: 'LEC 2025 Summer Season',     season: 'ALL' },
  { key: 'summerpo',  name: 'LEC 2025 Summer Playoffs',   season: 'ALL' },
];
const COMBINED_NAME = 'LEC 2025';
...
// roleMaps: splitKey → Map<playerNameLower, { role, team }>
...
// byPlayer: golggId → { name, country, role, rows: { splitKey: rowData & { team, role } } }
...
for (const split of SPLITS) {
  ...
  $s('table.table_list tbody tr').each((_, row) => {
    ...
    // Update role from latest split that has it
...
// ─── 3. Combined global + sub-combinés ────────────────────────────────────────
...
for (const [golggId, { name, country, role, rows }] of byPlayer) {
  ...
  // team = last split's team (most recent)
...
// Winter Combined (Winter + Winter Playoffs)
const winterCombPlayers = players.filter(p => p.rows.winter || p.rows.winterpo);
winterCombPlayers.forEach(p => {
  p.winterCombined = makeCombined([p.rows.winter, p.rows.winterpo].filter(Boolean));
});

// Spring Combined (Spring + Spring Playoffs)
const springCombPlayers = players.filter(p => p.rows.spring || p.rows.springpo);
springCombPlayers.forEach(p => {
  p.springCombined = makeCombined([p.rows.spring, p.rows.springpo].filter(Boolean));
});

// Summer Combined (Summer + Summer Playoffs)
const summerCombPlayers = players.filter(p => p.rows.summer || p.rows.summerpo);
summerCombPlayers.forEach(p => {
  p.summerCombined = makeCombined([p.rows.summer, p.rows.summerpo].filter(Boolean));
});

// ─── 4. LIR par split + combinés ─────────────────────────────────────────────
...
const winterCombResults = computeRatingsForGroup(winterCombPlayers, p => p.winterCombined);
winterCombPlayers.forEach((p, i) => { p.lirWinterCombined = winterCombResults[i]; });

const springCombResults = computeRatingsForGroup(springCombPlayers, p => p.springCombined);
springCombPlayers.forEach((p, i) => { p.lirSpringCombined = springCombResults[i]; });

const summerCombResults = computeRatingsForGroup(summerCombPlayers, p => p.summerCombined);
summerCombPlayers.forEach((p, i) => { p.lirSummerCombined = summerCombResults[i]; });
...
const exportData = {
  metadata: {
    ...
    tournaments: [{ name: COMBINED_NAME, league: 'LEC', year: 2025, split: 'Full Year', scrapedAt: new Date().toISOString() }],
  },
  ...
  players: players.map(p => {
    ...
    if (p.winterCombined) {
      const winterTeam = p.rows.winterpo?.team || p.rows.winter?.team || '';
      tournaments['LEC 2025 Winter'] = { ...toStats(p.winterCombined), team: winterTeam, ...(p.lirWinterCombined ?? {}) };
    }
    if (p.springCombined) {
      const springTeam = p.rows.springpo?.team || p.rows.spring?.team || '';
      tournaments['LEC 2025 Spring'] = { ...toStats(p.springCombined), team: springTeam, ...(p.lirSpringCombined ?? {}) };
    }
    if (p.summerCombined) {
      const summerTeam = p.rows.summerpo?.team || p.rows.summer?.team || '';
      tournaments['LEC 2025 Summer'] = { ...toStats(p.summerCombined), team: summerTeam, ...(p.lirSummerCombined ?? {}) };
    }
...
fs.writeFileSync(path.join(__dirname, '../../../frontend/public/leagues/lec-2025/export.json'), JSON.stringify(exportData, null, 2));

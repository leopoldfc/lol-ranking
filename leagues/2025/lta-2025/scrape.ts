/**
 * Scrape LTA 2025 depuis gol.gg
 * Splits : North Split 1 + N PO + North Split 2 + N PO + Championship
 * Combinés : North Combined, Combined annuel
 * Export → frontend/public/leagues/lta-2025/export.json
 *
 * Usage : npx tsx leagues/2025/lta-2025/scrape.ts
 */
...
const SPLITS = [
  { key: 'north1',    name: 'LTA 2025 North Split 1',           season: 'ALL' },
  { key: 'north1po',  name: 'LTA 2025 North Split 1 Playoffs',  season: 'ALL' },
  { key: 'north2',    name: 'LTA 2025 North Split 2',           season: 'ALL' },
  { key: 'north2po',  name: 'LTA 2025 North Split 2 Playoffs',  season: 'ALL' },
  { key: 'champ',     name: 'LTA 2025 Championship',            season: 'ALL' },
];
const COMBINED_NAME = 'LTA 2025';
...
// North Combined (N1 + N1 PO + N2 + N2 PO)
const northCombPlayers = players.filter(p => p.rows.north1 || p.rows.north1po || p.rows.north2 || p.rows.north2po);
northCombPlayers.forEach(p => {
  p.northCombined = makeCombined([p.rows.north1, p.rows.north1po, p.rows.north2, p.rows.north2po].filter(Boolean));
});
...
const northResults = computeRatingsForGroup(northCombPlayers, p => p.northCombined);
northCombPlayers.forEach((p, i) => { p.lirNorthCombined = northResults[i]; });
...
const exportData = {
  metadata: {
    ...
    tournaments: [{ name: COMBINED_NAME, league: 'LTA', year: 2025, split: 'Full Year', scrapedAt: new Date().toISOString() }],
  },
  players: players.map(p => {
    ...
    if (p.northCombined) {
      const northTeam = p.rows.north2po?.team || p.rows.north2?.team || p.rows.north1po?.team || p.rows.north1?.team || '';
      tournaments['LTA 2025 North Combined'] = { ...toStats(p.northCombined), team: northTeam, ...(p.lirNorthCombined ?? {}) };
    }

    tournaments[COMBINED_NAME] = { ...toStats(p.combined), team: p.team, ...(p.lirCombined ?? {}) };
...
fs.writeFileSync(path.join(__dirname, '../../../frontend/public/leagues/lta-2025/export.json'), JSON.stringify(exportData, null, 2));
